...
class DebateDialogue:
    ...
    async def initialize_async(self) -> Dict[str, Any]:
        ...
        # 캐시 확인 로직 추가 (이미 캐시가 적용되지 않은 경우에만)
        if not self.use_cache:
            title = self.room_data.get('title', '')
            context = self.room_data.get('context', '')
            
            # 디버그 로그 추가
            logger.info(f"[CACHE_DEBUG] Title: '{title}'")
            logger.info(f"[CACHE_DEBUG] Context length: {len(context) if context else 0}")
            logger.info(f"[CACHE_DEBUG] Context: '{context[:100]}...' " if context and len(context) > 100 else f"[CACHE_DEBUG] Context: '{context}'")
            
            if title:  # 제목이 있는 경우에만 캐시 확인
                logger.info(f"[CACHE_DEBUG] Checking cache for title: '{title}'")
                cached_entry = self._find_matching_cache_entry(title, context)
                
                if cached_entry:
                    logger.info(f"[CACHE_DEBUG] Found matching cache entry: {cached_entry.get('topic_id', 'unknown')}")
                    cache_applied = self._apply_cached_data(cached_entry)
                    
                    if cache_applied:
                        # 캐시가 성공적으로 적용된 경우, 에이전트만 다시 초기화하고 빠른 리턴
                        self.agents = self._initialize_agents()
                        self._prepare_moderator_opening_only()
                        
                        total_time = time.time() - start_time
                        logger.info(f"Fast initialization with cache completed in {total_time:.2f} seconds")
                        ...
                        if self.event_stream:
                            self.event_stream.emit_event(
                                InitializationEventType.COMPLETED,
                                {
                                    ...
                                    "cache_used": True,
                                    "cache_entry_id": cached_entry.get('topic_id', 'unknown')
                                }
                            )
                        ...
                        logger.warning("[CACHE_DEBUG] Cache application failed")
                else:
                    logger.info("[CACHE_DEBUG] No matching cache entry found")
            else:
                logger.info("[CACHE_DEBUG] No title provided, skipping cache check")
        else:
            logger.info("[CACHE_DEBUG] Cache already applied during __init__, using cached data")
            
            # 이미 캐시가 적용된 경우 빠른 리턴
            total_time = time.time() - start_time
            logger.info(f"Fast initialization with pre-applied cache completed in {total_time:.2f} seconds")
            ...
            if self.event_stream:
                self.event_stream.emit_event(
                    InitializationEventType.COMPLETED,
                    {
                        ...
                        "total_time": total_time,
                        "cache_used": True,
                        "cache_entry_id": self.cached_data.get('topic_id', 'unknown') if self.cached_data else 'unknown'
                    }
                )
        ...
        # 캐시가 없거나 적용 실패 시 기존 로직 실행
        logger.info("No cache found or cache application failed, proceeding with full initialization")
    ...
    async def _generate_stance_statements_async_enhanced(self, progress_tracker: Optional[TaskProgressTracker]) -> Dict[str, Any]:
        ...
        """
        try:
            # 캐시가 적용된 경우 캐시된 stance_statements 사용
            if self.use_cache and hasattr(self, 'stance_statements') and self.stance_statements:
                logger.info("Using cached stance statements in async method")
                
                if progress_tracker:
                    progress_tracker.start(["pro_stance", "con_stance"])
                    progress_tracker.update_subtask("pro_stance", "completed", {"stance": self.stance_statements.get("pro", "")})
                    progress_tracker.update_subtask("con_stance", "completed", {"stance": self.stance_statements.get("con", "")})
    ...
    def _generate_stance_statements(self) -> Dict[str, str]:
        """주제에서 찬성/반대 입장 명확화 및 컨텍스트 요약 생성"""
        # 캐시가 적용된 경우 캐시된 stance_statements 사용
        if self.use_cache and hasattr(self, 'stance_statements') and self.stance_statements:
            logger.info("Using cached stance statements")
            return self.stance_statements
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                if "pro" in stance_json and "con" in stance_json:
                    ...
                    # context가 있는 경우 context_summary도 생성
                    if context and context.strip():
                        self._generate_context_summary(context)
    ...
    def _generate_context_summary(self, context: str) -> None:
        """컨텍스트 요약 생성"""
        try:
            system_prompt = """
You are a helpful assistant that creates objective summaries of debate contexts.
Your task is to analyze the provided context and create a structured summary.
"""

            user_prompt = f"""
Please analyze the following debate context and create a structured summary:

CONTEXT:
{context}

Create a summary with the following structure:
1. A brief objective summary (2-3 sentences) 
2. Key points (3-5 bullet points)
3. Any relevant quotes or important details
...
Important: Be objective and neutral. Don't take sides in the debate.
"""
            ...
            if json_match:
                ...
                # context_summary 인스턴스 변수에 저장
                self.context_summary = {
                    'objective_summary': summary_json.get('summary', ''),  # ModeratorAgent가 기대하는 키로 변경
                    'bullet_points': summary_json.get('key_points', []),   # ModeratorAgent가 기대하는 키로 변경
                    'summary': summary_json.get('summary', ''),            # 호환성을 위해 유지
                    'key_points': summary_json.get('key_points', []),      # 호환성을 위해 유지
                    'relevant_quotes': summary_json.get('relevant_quotes', [])
                }
                
                logger.info("Successfully generated context summary")
            else:
                logger.warning("Failed to parse context summary response")
                self.context_summary = {}
    ...
    def _prepare_moderator_opening_only(self) -> None:
        ...
        try:
            # 캐시된 오프닝 메시지가 있는 경우 사용
            if self.use_cache and hasattr(self, 'cached_opening_message') and self.cached_opening_message:
                logger.info("Using cached opening message for moderator")
                # 캐시된 메시지를 모더레이터 에이전트에 설정
                moderator_agent = self.agents.get(ParticipantRole.MODERATOR)
                if moderator_agent and hasattr(moderator_agent, 'set_prepared_opening'):
                    moderator_agent.set_prepared_opening(self.cached_opening_message)
                elif moderator_agent:
                    # 일반적인 방법으로 캐시된 메시지 저장
                    moderator_agent._cached_opening_message = self.cached_opening_message
                return
    ...
    def _check_and_apply_cache(self) -> None:
        """초기화 시 캐시 확인 및 적용"""
        title = self.room_data.get('title', '')
        context = self.room_data.get('context', '')
        
        # 디버그 로그
        logger.info(f"[INIT_CACHE_DEBUG] Title: '{title}'")
        logger.info(f"[INIT_CACHE_DEBUG] Context length: {len(context) if context else 0}")
        
        if title:  # 제목이 있는 경우에만 캐시 확인
            logger.info(f"[INIT_CACHE_DEBUG] Checking cache for title: '{title}'")
            cached_entry = self._find_matching_cache_entry(title, context)
            
            if cached_entry:
                logger.info(f"[INIT_CACHE_DEBUG] Found matching cache entry: {cached_entry.get('topic_id', 'unknown')}")
                cache_applied = self._apply_cached_data(cached_entry)
                
                if cache_applied:
                    logger.info("[INIT_CACHE_DEBUG] Cache successfully applied!")
                else:
                    logger.warning("[INIT_CACHE_DEBUG] Cache application failed")
            else:
                logger.info("[INIT_CACHE_DEBUG] No matching cache entry found")
        else:
            logger.info("[INIT_CACHE_DEBUG] No title provided, skipping cache check")
    
    def _load_pregenerated_cache(self) -> Optional[Dict[str, Any]]:
        """pregenerated_debates.json 파일에서 캐시 데이터를 로드합니다."""
        try:
            # 현재 파일 위치 기준으로 절대 경로 계산
            current_file = Path(__file__)  # src/dialogue/types/debate_dialogue.py
            project_root = current_file.parent.parent.parent.parent  # sapiens_engine 루트
            cache_file_path = project_root / "src" / "new" / "data" / "pregenerated_debates.json"
            
            logger.info(f"[CACHE_LOAD_DEBUG] Looking for cache file at: {cache_file_path}")
            
            if not cache_file_path.exists():
                logger.info(f"[CACHE_LOAD_DEBUG] Cache file not found at: {cache_file_path}")
                return None
            
            with open(cache_file_path, 'r', encoding='utf-8') as f:
                cache_data = json.load(f)
            
            logger.info(f"[CACHE_LOAD_DEBUG] Successfully loaded cache with {len(cache_data.get('topics', {}))} topics")
            return cache_data
            
        except Exception as e:
            logger.error(f"[CACHE_LOAD_DEBUG] Error loading pregenerated cache: {str(e)}")
            return None
    
    def _find_matching_cache_entry(self, title: str, context: str) -> Optional[Dict[str, Any]]:
        """제목과 컨텍스트가 일치하는 캐시 엔트리를 찾습니다."""
        try:
            logger.info(f"[CACHE_MATCH_DEBUG] Starting cache search for title: '{title}'")
            logger.info(f"[CACHE_MATCH_DEBUG] Context length: {len(context) if context else 0}")
            
            cache_data = self._load_pregenerated_cache()
            if not cache_data or 'topics' not in cache_data:
                logger.info("[CACHE_MATCH_DEBUG] No cache data or topics found")
                return None
            
            logger.info(f"[CACHE_MATCH_DEBUG] Cache has {len(cache_data.get('topics', {}))} topics")
            ...
            logger.info(f"[CACHE_MATCH_DEBUG] Normalized title: '{normalized_title}'")
            logger.info(f"[CACHE_MATCH_DEBUG] Normalized context length: {len(normalized_context)}")
            
            # 각 토픽과 비교
            for topic_id, topic_data in cache_data['topics'].items():
                cache_title = topic_data.get('title', '')
                cache_context = ""
                
                # 컨텍스트 추출 (original_data.context.content 또는 다른 경로)
                if 'original_data' in topic_data and 'context' in topic_data['original_data']:
                    context_data = topic_data['original_data']['context']
                    if isinstance(context_data, dict):
                        cache_context = context_data.get('content', '')
                    else:
                        cache_context = str(context_data)
                
                normalized_cache_title = normalize_text(cache_title)
                normalized_cache_context = normalize_text(cache_context)
                
                # 제목 일치 확인
                title_match = normalized_title == normalized_cache_title
                
                if title_match:
                    logger.info(f"[CACHE_MATCH_DEBUG] ✅ Title match found! Topic ID: {topic_id}")
                    logger.info(f"[CACHE_MATCH_DEBUG] Cache title: '{cache_title}'")
                    
                    # 컨텍스트 일치 확인 (둘 다 비어있거나 일치하는 경우)
                    context_match = (
                        (not normalized_context and not normalized_cache_context) or
                        (normalized_context and normalized_cache_context and 
                         normalized_context in normalized_cache_context or 
                         normalized_cache_context in normalized_context)
                    )
                    
                    logger.info(f"[CACHE_MATCH_DEBUG] Context match: {context_match}")
                    
                    if context_match:
                        logger.info(f"[CACHE_MATCH_DEBUG] 🎉 Complete match found: {topic_id}")
                        return topic_data
                    else:
                        logger.info(f"[CACHE_MATCH_DEBUG] ❌ Context mismatch for topic {topic_id}")
                        logger.info(f"[CACHE_MATCH_DEBUG] Input context preview: '{normalized_context[:100]}...'")
                        logger.info(f"[CACHE_MATCH_DEBUG] Cache context preview: '{normalized_cache_context[:100]}...'")
            
            logger.info("[CACHE_MATCH_DEBUG] No matching cache entry found after checking all topics")
            return None
            
        except Exception as e:
            logger.error(f"[CACHE_MATCH_DEBUG] Error finding matching cache entry: {str(e)}")
            return None
    
    def _apply_cached_data(self, cached_entry: Dict[str, Any]) -> bool:
        """캐시된 데이터를 인스턴스 속성에 적용합니다."""
        try:
            generated_data = cached_entry.get('generated_data', {})
            
            # 찬반 입장 진술문 적용
            stance_statements = generated_data.get('stance_statements', {})
            if stance_statements:
                self.stance_statements = {
                    'pro': stance_statements.get('pro', ''),
                    'con': stance_statements.get('con', '')
                }
                logger.info("Applied cached stance statements")
            
            # 컨텍스트 요약 적용
            context_summary = generated_data.get('context_summary', {})
            if context_summary:
                self.context_summary = {
                    'objective_summary': context_summary.get('summary', ''),  # ModeratorAgent가 기대하는 키로 변경
                    'bullet_points': context_summary.get('key_points', []),   # ModeratorAgent가 기대하는 키로 변경
                    'summary': context_summary.get('summary', ''),            # 호환성을 위해 유지
                    'key_points': context_summary.get('key_points', []),      # 호환성을 위해 유지
                    'relevant_quotes': context_summary.get('relevant_quotes', [])
                }
                logger.info("Applied cached context summary")
            
            # 모더레이터 오프닝 메시지 적용 (있는 경우)
            opening_message = generated_data.get('opening_message', '')
            if opening_message:
                self.cached_opening_message = opening_message
                logger.info("Applied cached opening message")
            
            self.use_cache = True
            self.cached_data = cached_entry
            
            logger.info("Successfully applied cached data")
            return True
